...
def process():
    ...
    for index, name in ordered.items():
        ...
        if display is not None and game_strings.has_ability(display):
            _abilities[index] = "pokemon.ability." + game_strings.clean_up(display)
            continue

        lookup_key = name.lower().replace("_", "")
        if lookup_key in lookup:
            _abilities[index] = "pokemon.ability." + lookup[lookup_key]
        else:
            if display is None:
            ...
            _abilities[index] = key

        _ability_name_by_constant[name] = _abilities[index]
